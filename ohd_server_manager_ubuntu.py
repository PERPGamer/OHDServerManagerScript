...
# Ubuntu-compatible OHD Server Manager with auto SteamCMD download and safe privilege handling
# Combined final version generated by assistant for user
...
# ============================================================
# PRIVILEGE HANDLING SECTION
# ============================================================

def drop_privileges(user="steam"):
    ...
    if os.environ.get("ALLOW_ROOT", "") == "1":
        os.environ["SteamCMDForceRunAsRoot"] = "1"
        print("[WARN] Running as root (ALLOW_ROOT=1 set). SteamCMDForceRunAsRoot=1 enabled.")
        return
    ...
    try:
        ...
        print(f"[INFO] Dropped privileges to user '{user}' (uid={uid}).")
    except KeyError:
        print(f"[ERROR] User '{user}' not found.")
        print(f"Create it first: sudo useradd -m {user}")
        sys.exit(1)
...
# Call early to prevent SteamCMD permission issues
try:
    if os.geteuid() == 0:
        drop_privileges("steam")
except Exception:
    # geteuid may not exist on some platforms; ignore in that case
    pass
